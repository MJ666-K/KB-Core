...
import { kgSearchNodesTool, kgGetNodeTool, kgNeighborsTool, kgPathTool, kgSubgraphTool, kgToChunkTool } from '@features/kg/tools';
import { ingestKgData } from '@features/kg/ingest';
...
app.get('/stats', (c) => kgRoute(async () => {
  if (!config.kgEnabled) return c.json({ error: 'Knowledge graph is disabled' }, 503);
  const { withSession } = await import('@features/kg/client');
